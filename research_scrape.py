...
def _age(date_str):
    """'2026-09-14 · 8 days ago' — saves doing the date math when deciding
...
def _render_source(source_data, notes):
    ...
    if note.get("synthesis"):
        ...
        meta = f"synthesized {_age(as_of)}" if as_of else ""
        return (
            f"<div class='article-entry'><h3><a href='{site_url}'>{label_html}</a></h3>"
    ...
    return (
        ...
        f"<p class='metadata'>{label_html} &nbsp;·&nbsp; posted {_age(item.get('date', ''))}</p>"
        f"{trail_html}</div>"
    )
